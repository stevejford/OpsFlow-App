...
import EditInductionModal from '@/components/inductions/EditInductionModal';
import ContinueInductionModal from '@/components/inductions/ContinueInductionModal';
...
export default function InductionTrackingClient({
  ...
  const isEditModalOpen = Boolean(searchParams.get('edit'));
  const isContinueModalOpen = Boolean(searchParams.get('continue'));
  ...
  useEffect(() => {
    ...
    const targetId = viewId || rescheduleId || editId || continueId;
  ...
  const handleContinueInduction = (inductionId: string) => {
    // Navigate to continue page using URL parameter
    router.push(`/induction-tracking?continue=${inductionId}`);
  };
  
  // Handle continue induction submission
  const handleContinueSubmit = async (inductionId: string, progress: number) => {
    try {
      const response = await fetch(`/api/inductions/${inductionId}/progress`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleEditInduction = (inductionId: string) => {
    // Navigate to edit page using URL parameter
    router.push(`/induction-tracking?edit=${inductionId}`);
  };
  
  // Handle edit induction submission
  const handleEditSubmit = async (updatedInduction: Partial<Induction>) => {
    try {
      const response = await fetch(`/api/inductions/${updatedInduction.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(updatedInduction),
      });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              <Icon name="fas fa-upload" className="mr-1.5" />Bulk Import
            </button>
            <button 
            ...
            >
              <Icon name="fas fa-download" className="mr-1.5" />Export
            </button>
            <button 
            ...
            >
              <Icon name="fas fa-plus" className="mr-1.5" />Add Induction
            </button>
          </div>
        </div>
      </div>
      ...
      {selectedInduction && searchParams.get('reschedule') && (
      ...
      {selectedInduction && isDetailsModalOpen && (
        <InductionDetails 
          induction={selectedInduction}
      ...
      {/* EditInductionModal - Using URL-based state management */}
      {selectedInduction && isEditModalOpen && (
        <EditInductionModal 
          induction={selectedInduction}
          onClose={() => router.push('/induction-tracking')}
      ...
      {/* ContinueInductionModal - Using URL-based state management */}
      {selectedInduction && isContinueModalOpen && (
        <ContinueInductionModal 
          induction={selectedInduction}
          onClose={() => router.push('/induction-tracking')}
